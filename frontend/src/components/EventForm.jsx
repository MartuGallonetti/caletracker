...
export default function EventForm({ onEventCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        // Pasamos data.evento porque tu backend devuelve { mensaje, evento }
  ...
  return (
    <form onSubmit={handleSubmit} className="p-5 bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col gap-4">
      <h3 className="text-lg font-bold text-gray-800 border-b pb-2">Crear Nuevo Evento</h3>
      ...
      <button type="submit" className="bg-blue-600 text-white p-2 rounded-lg font-bold hover:bg-blue-700 transition mt-2 text-sm">
        Guardar Evento
      </button>
    </form>
  );
